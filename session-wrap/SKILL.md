...
### 3 — Memory staleness audit (forcing function)
Inspect every memory file **body**, not just index lines or filenames — a
filename scan misses memories whose bodies reference now-changed state
(merged PRs, completed "pending" actions, "in progress" claims, renamed
fields, "as of <date>" snapshots older than this session). For EACH memory,
edit the body/description to current reality if needed.

**Required output in your response: a per-memory disposition list** — one line
per memory inspected, each marked `unchanged` / `body-updated` /
`index-updated` / `flagged-for-deletion`. If you cannot produce this list you
did not do the audit.

### 4 — Deletion candidates
Surface (do not delete) memories or sections that have outlived use:
superseded snapshots, vigilance notes for failure modes now structurally
prevented, spent per-iteration artifacts. For each: file, one-line content,
why eligible, risk of keeping, where the value survives otherwise.
...
### 5 — Loose threads & informal decisions
...
### 6 — Git hygiene (verify, never assert)
...
| Step 3 done as a filename/topic scan | Inspect file *bodies*. The per-memory disposition list is the proof. |
| "No memory candidates" without looking | Actually inspect first; then "no candidates" is allowed. |
...
All 7 steps done. Next session can start from CLAUDE.md + project docs +
